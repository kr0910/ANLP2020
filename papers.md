...
- 編集距離(レーベンシュタイン距離)：1文字の挿入・削除・置換によって、一方の文字列をもう一方の文字列に変形するのに必要な手順の最小回数として定義
...
- 概要
	- Dense Video Captioningの自動評価で再現率だけでなく精度ちゃんと見れる指標を提案
		- Dense Video Captioning:動画中の場面に応じて平均3から4文程度のキャプションで表現するタスク
		- METEORスコアは対応の取れる文で評価するので再現率としてはいいが、生成文が多すぎてもいい評価となってしまう問題点がある
	- ActivityNetデータセットに対して各動画中の場面に人間がキャプションを付与したDense Video Captioningデータセットを使用した
	- キャプション生成には動画の対応する時間の開始位置と終わり位置が出力されるようになっており、それのIoUでスコアが一定以上のものを対応がとれたものとしてMETEORを測る
	- 以下の制約を加えた
		- 生成キャプション文に対応する正解キャプション文の数を高々1つであること
		- 時間順序に応じて対応すること
	- 生成キャプション文と正解キャプション文との間のIoUのスコアの和が最大となるよう対応関係を決定
	- IoUスコアを指標として**動的計画法により**正解キャプションについての**最適な対応関係を決定する**
		- これによって最適な対応関係を作ることができたので、それらについて精度・再現率を計算できる
- METEORスコア

### P1-31,運転免許試験自動解答における問題解説文の利用,"○的場成紀, 古賀雅樹, 吉村優志, 田邉豊 (大阪工大), 小林一郎 (お茶大), 平博順 (大阪工大)"
- 概要
	- 普通自動車免許の試験問題の自動解答を含意関係認識として解く方法を検証
	- word2vecとBERTの２つで検証
	- 提案としては問題集の解説記事を参照としてQAを解く
	- word2vecは文ベクトルのcos類似度で閾値0.95以上だったものは含意関係ありとして問題文の正誤を判定した
	- BERTは含意認識モード？で行った？？？
	- 正解率は62%
	- word2vecでは数字の計算が必要なものに強かった
	- BERTは言い換えたものに強かった

### P1-32,介護用対話システムのための高齢者の発話理解,"○淺尾仁彦, Julien Kloetzer, 水野淳太 (NICT), 齊木大 (日本総研), 門脇一真 (NICT/日本総研), 鳥澤健太郎 (NICT/NAIST)"
- 概要
	- 高齢者の発話を理解するためにBERT(LARGE)をベースに以下を提案
		1. 介護質問応答分類モジュール
			- こちらが投げた質問に対する回答を「YES, NO, UNK（ユーザが答えを知らない）,NEGP(質問の前提の否定),OTHER」に分類
			- 平均精度89.0%
		2. 介護情報含意認識モジュール
			- 質問に対する回答に、ほかの話題などの情報が含まれていた場合にそれを検知する
			- 平均精度87.1%
	- 回答と質問は[sep]で区切ってBERTに入れた
	- 含意認識は1,206クラスの二値マルチラベル分類問題とした
	- two-segmentなるマルチラベル分類に変わる手法を取っているがよくわからない
- データセット構築
	- 応答分類のためのデータ構築
		- 34人で行った
		- 一つの会話に3人でやって2人以上でラベルを採用。その他は破棄
		- Fleiss'κは0.736と高い
	- 含意認識のためのデータ構築
		- 回答に二つの意味が含まれるような場合、つまり(U->B)=>(U->A)∩(A->B)と分解できるものも抽出したい
		- 類似度を高い二つの文の含意関係の有無をアノテーションしてそれらをマージすることで2つの含意を含む文を作成した
		- (i)内容語のオーバーラップと(ii)トピックワードモデルを用いた指標を用いて類似度を評価

### P1-33,語りの傾聴において表出する応答データの拡充,"○村田匡輝 (豊田高専), 大野誠寛 (東京電機大), 松原茂樹 (名大)"
- 概要
	- 傾聴応答のデータ集計
		- 傾聴応答のラベルとしては17個（相槌、関心、同意、納得、etc)
	- 傾聴応答は作業者ごとに違ったタイミング・言葉になることが多く、同じ言葉でも異なるラベルがつくこともあった。
- データは高齢者が10個の質問に答えた音声データを書き起こした64,541個の傾聴応答

### P1-34,UD Japanese GSD の再整備と固有表現情報付与,"○松田寛 (リクルート), 若狭絢 (国語研), 山下華代 (フリー), 大村舞, 浅原正幸 (国語研)"
- 概要
	- UD Japanese GSDの再整備、新たに固有表現情報を付与した
	- **固有表現情報が加わることでspaCy標準日本語モデルへの依存構造解析・固有表現抽出モデルの搭載が可能になる**
- Universal Dependencies (UD)：多言語間で共通のアノテーション方式を用いて係り受けのツリーバンクを開発する国際プロジェクト
- [UD Japanese GSD](https://github.com/UniversalDependencies/UD_Japanese-GSD)：元テキストとしてwikipediaのテキストを含んでいる
- UD Japanese BCCWJ：日本語依存構造解析器GiNZAの学習元ファイル
...
- 概要
	- 単語の埋め込みをベクトルではなくガウス分布に埋め込む領域表現が注目されている
		- Vilnis and McCallum:各単語をガウス分布でモデル化し，コーパスから分布の平均と分散を学習するw2g法を提案した
		- Athiwaratkun and Wilson:その発展として各単語を混合ガウス分布によってモデル化するw2gm法により各単語に対して複数の語義を割り当てることを可能にした
			- 単語間の意味的類似度推定タスクなどのベンチマークで単語分散表現よりも高い性能を達成した
			- どちらの手法も全ての単語に対して同じ語義数を割り当てる
			- 訓練時に固定長の窓幅の文脈しか考慮しないという課題
- キーワード:
